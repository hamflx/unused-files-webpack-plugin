...
async function applyAfterEmit(compiler, compilation, plugin) {
  try {
    ...
    if (plugin.options.output) {
      await _fs.default.promises.writeFile(plugin.options.output, unusedFileContents);
    }
...
`);
  ...
  apply(compiler) {
    if (compiler.hooks && compiler.hooks.afterEmit && compiler.hooks.afterEmit.tapPromise) {
      ...
      compiler.plugin(`after-emit`, (compilation, done) => applyAfterEmit(compiler, compilation, this).then(done, done));
    }
  }
